...
const createStylesProps = <T extends ComponentType>(
  ...
  return styled(element)<StyledProps>`
    ...
    ${({$bw}: StyledProps) =>
      isDefined($bw) && `border-width: ${getUnit($bw)};`}
    ${({$bs}: StyledProps) =>
      isDefined($bs) && `border-style: ${$bs ?? 'solid'};`}
    ...
    ${({$pb}: StyledProps) =>
      isDefined($pb) && `padding-bottom: ${getUnit($pb)};`}
    ${({$pl}: StyledProps) =>
      isDefined($pl) && `padding-left: ${getUnit($pl)};`}
    ${({$pr}: StyledProps) =>
      isDefined($pr) && `padding-right: ${getUnit($pr)};`}
    ...
    ${({$mb}: StyledProps) =>
      isDefined($mb) && `margin-bottom: ${getUnit($mb)};`}
    ${({$ml}: StyledProps) => isDefined($ml) && `margin-left: ${getUnit($ml)};`}
    ${({$mr}: StyledProps) =>
      isDefined($mr) && `margin-right: ${getUnit($mr)};`}
